...
export function Summary({items = [], onOrderSubmit}) {
    ...
    return (
        <>
            ...
            <hr/>
            {items.map(item => <SummaryItem key={item.productId} productName={item.productName} count={item.count}/>)}
            <form>
                <div className="mb-3">
                    <label htmlFor="email" className="form-label">이메일</label>
                    <input type="email" className="form-control mb-1" value={order.email} onChange={changeEmailInput}
                           id="email"/>
                </div>
                <div className="mb-3">
                    <label htmlFor="address" className="form-label">주소</label>
                    <input type="text" className="form-control mb-1" value={order.address} onChange={changeAddressInput}
                           id="address"/>
                </div>
                <div className="mb-3">
                    <label htmlFor="postcode" className="form-label">우편번호</label>
                    <input type="text" className="form-control" value={order.postcode} onChange={changePostcodeInput}
                           id="postcode"/>
                </div>
